...
struct HighResolutionAnalysisTests {
    ...
    func testSettingsManagerHighQualityDefaults() async throws {
        await MainActor.run {
            ...
            if currentProStatus {
                #expect(
                    settingsManager.isHighQualityToggleAvailable == true,
                    "Pro users should have toggle available")
            } else {
                #expect(
                    settingsManager.isHighQualityToggleAvailable == false,
                    "Non-Pro users should not have toggle available")
            }
        }
    }
        ...
        await MainActor.run {
            ...
            if settingsManager.isPro && settingsManager.highQualityAnalysisEnabled {
                #expect(
                    ...
                    "Pro users with high quality should use 1250px resolution")
            } else {
                #expect(
                    settingsManager.effectiveImageResolution == 512.0,
                    "Standard users should use 512px resolution")
            }
        }
    }
        ...
        await MainActor.run {
            ...
            if settingsManager.isPro && settingsManager.highQualityAnalysisEnabled {
                #expect(
                    ...
                    "Pro users with high quality should use high detail")
            } else {
                #expect(
                    settingsManager.effectiveDetailLevel == "low", "Standard users should use low detail")
            }
        }
    }
    ...
    func testOptimizedImageManagerDualResolution() async throws {
        ...
        let standardBase64 = await imageManager.prepareImageForAI(
            from: testImage, useHighQuality: false)
        #expect(standardBase64 != nil, "Standard quality image preparation should succeed")

        // Test high quality (1250px max)
        let highQualityBase64 = await imageManager.prepareImageForAI(
            from: testImage, useHighQuality: true)
    ...
    func testSettingsPersistence() async throws {
        await MainActor.run {
            ...
            #expect(
                settingsManager.highQualityAnalysisEnabled == !originalValue,
                "High quality setting should toggle")
    ...
    func testProVsNonProBehavior() async throws {
        await MainActor.run {
            ...
            if settingsManager.isPro {
                ...
                #expect(
                    settingsManager.isHighQualityToggleAvailable == true,
                    "Pro users should have access to high quality toggle")
    ...
    func testSettingsReset() async throws {
        await MainActor.run {
            ...
            #expect(
                settingsManager.highQualityAnalysisEnabled == false,
                "High quality should be disabled after reset")
    ...
    func testImageQualityFeatureIntegration() async throws {
        ...
        if shouldTest {
            ...
            let result = await imageManager.prepareImageForAI(
                from: testImage, useHighQuality: useHighQuality)
