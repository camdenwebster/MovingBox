...
struct ProgressMapperTests {
    ...
    func exportProgressIncreasesMonotonically() {
        ...
        for phase in phases {
            ...
            #expect(mapped.progress >= lastProgress, "Progress should never decrease. Got \(mapped.progress) after \(lastProgress)")
    ...
    func exportWeightsSumToOne() {
        ...
        let sum = weights.dataFetching + weights.csvWriting + 
                  weights.photoCopying + weights.archiving
    ...
    func importWeightsSumToOne() {
        ...
        let sum = weights.unzipping + weights.readingCSV + 
                  weights.processingData + weights.copyingPhotos
    ...
    func progressPhasesUseExpectedRanges() {
        let dataFetchingEnd = ProgressMapper.mapExportProgress(.fetchingData(phase: "test", progress: 1.0))
