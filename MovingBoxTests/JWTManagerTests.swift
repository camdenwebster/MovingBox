...
struct JWTManagerTests {
    ...
    func tokenHasValidHeader() throws {
        ...
        let token = jwtManager.generateToken()
        let components = token.components(separatedBy: ".")
        let paddedHeader = components[0].padding(
            toLength: ((components[0].count + 3) / 4) * 4, withPad: "=", startingAt: 0)
        ...
        guard let header = try? JSONSerialization.jsonObject(with: headerData) as? [String: String]
        else {
    ...
    func tokenHasValidPayload() throws {
        ...
        let token = jwtManager.generateToken()
        let components = token.components(separatedBy: ".")
        let paddedPayload = components[1].padding(
            toLength: ((components[1].count + 3) / 4) * 4, withPad: "=", startingAt: 0)
    ...
    func signatureIsValid() throws {
        ...
        let paddedSignature = components[2].padding(
            toLength: ((components[2].count + 3) / 4) * 4, withPad: "=", startingAt: 0)
