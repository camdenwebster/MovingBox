...
final class InventoryListDeletionUITests: XCTestCase {
    ...
    func testEnterAndExitSelectionMode() throws {
        ...
        XCTAssertFalse(
            inventoryListScreen.isSelectionModeActive(), "Selection mode should not be active initially")
        ...
        XCTAssertTrue(
            inventoryListScreen.isSelectionModeActive(), "Selection mode should be active after entering")
        ...
        XCTAssertFalse(
            inventoryListScreen.isSelectionModeActive(), "Selection mode should be inactive after exiting"
        )
    }
    ...
    func testSelectAndDeleteSingleItem() throws {
        ...
        XCTAssertTrue(
            inventoryListScreen.waitForDeleteConfirmationAlert(),
        ...
        XCTAssertFalse(
            inventoryListScreen.isSelectionModeActive(), "Selection mode should exit after deletion")
    }
    ...
    func testSelectAndDeleteMultipleItems() throws {
        ...
        XCTAssertGreaterThan(
            initialItemCount, 2, "Should have at least 3 items to test multiple deletion")
        ...
        let alertMessage = inventoryListScreen.deleteConfirmationAlert.staticTexts.element(boundBy: 1)
            .label
        ...
        let finalItemCount = inventoryListScreen.getItemCount()
        XCTAssertEqual(
            finalItemCount, initialItemCount - 2, "Item count should decrease by 2 after deletion")
    ...
    func testCancelDeletion() throws {
        ...
        let finalItemCount = inventoryListScreen.getItemCount()
        XCTAssertEqual(
            finalItemCount, initialItemCount, "Item count should remain the same after canceling deletion"
        )

        // And: We should still be in selection mode
        XCTAssertTrue(
            inventoryListScreen.isSelectionModeActive(),
            "Selection mode should remain active after canceling deletion")
    }
    ...
    func testDeleteWithNoItemsSelected() throws {
        ...
        XCTAssertFalse(
            deleteButton.isHittable, "Delete button should be disabled when no items are selected")
