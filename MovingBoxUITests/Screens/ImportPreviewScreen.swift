...
class ImportPreviewScreen {
    ...
    init(app: XCUIApplication) {
        ...
        self.itemsCount =
            app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Items'")).firstMatch
        self.locationsCount =
            app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Locations'")).firstMatch
        self.labelsCount =
            app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Labels'")).firstMatch
    }
    ...
    func waitForErrorState() -> Bool {
        let errorIcon = app.images.matching(NSPredicate(format: "identifier CONTAINS 'xmark'"))
            .firstMatch
        return errorIcon.waitForExistence(timeout: 10)
    }

    func getErrorMessage() -> String {
        let errorText = app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Import Failed'"))
            .firstMatch
