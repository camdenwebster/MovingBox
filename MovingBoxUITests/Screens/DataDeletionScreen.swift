...
class DataDeletionScreen {
    ...
    init(app: XCUIApplication) {
        ...
        self.warningDescription =
            app.staticTexts["This will permanently delete all your inventory data including:"]
    ...
    func isDisplayed() -> Bool {
        return app.navigationBars["Delete All Data"].waitForExistence(timeout: 5) && warningLabel.exists
    }
    ...
    func isWarningSectionVisible() -> Bool {
        return warningLabel.exists && warningDescription.exists && inventoryItemsWarning.exists
            && locationsWarning.exists && labelsWarning.exists && homeInfoWarning.exists
    }

    func isScopeSectionVisible() -> Bool {
        return deletionScopeHeader.exists && localOnlyOption.exists && localAndICloudOption.exists
            && localOnlyDescription.exists && localAndICloudDescription.exists
    }

    func isConfirmationSectionVisible() -> Bool {
        return confirmationInstructions.exists && confirmationTextField.exists && deleteButton.exists
