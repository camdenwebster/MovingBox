    ...
    func findLatestDownloadedZip() -> URL? {
        ...
        do {
            ...
            let sortedByDate = try zipFiles.sorted { url1, url2 in
                let date1 =
                    try url1.resourceValues(forKeys: [.contentModificationDateKey]).contentModificationDate
                    ?? Date(timeIntervalSince1970: 0)
                let date2 =
                    try url2.resourceValues(forKeys: [.contentModificationDateKey]).contentModificationDate
                    ?? Date(timeIntervalSince1970: 0)
                return date1 > date2
            }
