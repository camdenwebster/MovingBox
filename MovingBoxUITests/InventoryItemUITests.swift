...
final class InventoryItemUITests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        app.launch()
        XCTAssertTrue(
            dashboardScreen.allInventoryButton.waitForExistence(timeout: 15), "App did not launch in time"
        )
    }
    ...
    func testAddItemFromPhotoViaListView() throws {
        ...
        XCTAssertTrue(
            detailScreen.titleField.waitForNonExistence(timeout: 5),
            "Camera should be ready after permissions")
    }
    ...
    //        XCTAssertTrue(
    //            detailScreen.tapToAddPhotoButton.waitForExistence(timeout: 5),
    ...
    //        XCTAssertTrue(
    //            detailScreen.analyzeWithAiButton.waitForExistence(timeout: 5),
    ...
    func testAddItemViaDashboard() throws {
        ...
        // And: User taps the add item button from dashboard and takes a photo
        openCamera()
        cameraScreen.takePhoto()

        // When: User submits the photo for analysis
        cameraScreen.doneButton.tap()

        // Then: Detail view should appear after AI analysis completes
        XCTAssertTrue(
            detailScreen.titleField.waitForExistence(timeout: 10),
            "Detail view should appear after AI analysis")
