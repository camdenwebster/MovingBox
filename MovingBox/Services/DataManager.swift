...
actor DataManager {
    ...
    struct SendableError: Sendable {
        ...
        var errorDescription: String? {
            if description.contains("nothingToExport") {
                ...
                return
                    "The selected file is not a valid ZIP archive. Please choose a valid MovingBox export file."
        ...
        var recoverySuggestion: String? {
            if description.contains("nothingToExport") {
                ...
                return
                    "Try moving the file to a different location or grant the app permission to access it."
            } else if description.contains("fileTooLarge") {
    ...
    nonisolated func exportInventoryWithProgress(
        ...
        config: ExportConfig = ExportConfig(
            includeItems: true, includeLocations: true, includeLabels: true)
            ...
            Task { @MainActor in
                do {
                    ...
                    if config.includeLocations {
                        continuation.yield(
                            .fetchingData(
                                phase: "locations", progress: Double(completedSteps) / Double(totalSteps)))
                    ...
                    let archiveName =
                        fileName
                        ?? "MovingBox-export-\(DateFormatter.exportDateFormatter.string(from: .init()))"
                    ...
                    continuation.yield(
                        .creatingArchive(progress: Double(completedSteps) / Double(totalSteps)))
                    ...
                    let archiveURL = try await createArchive(
                        ...
                        progressHandler: { filesProcessed, totalFiles in
                            ...
                            let overallProgress =
                                (Double(completedSteps) + archivePhaseProgress) / Double(totalSteps)
        ...
        AsyncStream { continuation in
            Task.detached(priority: .userInitiated) {
                do {
                    ...
                    await MainActor.run {
                        if config.includeLocations {
                            if let existingLocations = try? modelContext.fetch(
                    ...
                    if !imageCopyTasks.isEmpty {
                        ...
                        for batchStart in stride(from: 0, to: imageCopyTasks.count, by: imageBatchSize) {
                            ...
                            let copyResults = try await withThrowingTaskGroup(of: (Int, URL?, URL?).self) {
                                group in
    ...
    /// - Note: Must be called on MainActor since it creates SwiftData model objects
    @MainActor
    private func createAndConfigureLabel(name: String, desc: String, colorHex: String, emoji: String)
        -> InventoryLabel
    {
    ...
    func exportSpecificItems(
        items: [InventoryItem], modelContainer: ModelContainer, fileName: String? = nil
    ) async throws -> URL {
