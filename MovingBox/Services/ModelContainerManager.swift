...
class ModelContainerManager {
    ...
    var cloudKitSyncMessage: String = "Syncing with iCloud..."
        ...
        do {
            ...
            if error.domain == "SwiftData.SwiftDataError" {
                ...
                do {
                    ...
                    print(
                        "⚠️ ModelContainerManager - Schema recovery performed. Old data backed up to: \(backupURL.path)")
    ...
    private func migrateToSyncContainer(from localContainer: ModelContainer, to syncContainer: ModelContainer)
        async throws
    {
    ...
    internal func performMultiHomeMigration() async throws {
        ...
        for item in items {
            ...
            let needsMigration =
                (item.location == nil && item.home == nil) || (item.location != nil && item.location?.home == nil)
    ...
    func setSyncEnabled(_ enabled: Bool) {
        ...
        print(
            "📦 ModelContainerManager - Sync \(enabled ? "enabled" : "disabled"). App restart required for full effect.")
    }
